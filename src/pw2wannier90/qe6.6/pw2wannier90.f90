...
PROGRAM pw2wannier90
  ...
  CALL mp_bcast(nscf_sym,ionode_id, world_comm)
...
!  CALL errore("pw2wannier90","Spinorbit without spinor=T",1)
  ...
  IF (ionode) THEN     ! read from ionode only
     CALL scan_file_to('bvectors',found)
     IF (found) THEN
       DO i=1, nnb
  ...
  ! bvec in Ang-1 => in unit of 2pi/a
  bvec = bvec / tpiba * bohr
  ! bvec in unit of 2pi/a => xbvec in crystal coord
  xbvec = bvec
  CALL cryst_to_cart(nnb, xbvec, at, -1)
...
SUBROUTINE compute_mmn_sym
   ...
   USE mp_pools,        ONLY : intra_pool_comm
   USE spin_orb,        ONLY : lspinorb
   USE wannier,         ONLY : iknum, ikstart, nnb, nexband, excluded_band, seedname, bvec
   !
   IMPLICIT NONE
   !
   INTEGER, EXTERNAL      :: find_free_unit
   ...
   iun_mmn = find_free_unit()
   CALL date_and_tim( cdate, ctime )
   header='Symmetrized Mmn created on '//cdate//' at '//ctime
   IF (ionode) THEN
      ...
      DO ik=1, nkstot
         ...
         DO isym=1, nsym2
            ...
            DO m=1, nbnd
               ...
               DO n=1, nbnd
                 IF (excluded_band(n)) CYCLE
                 IF (abs(et(m,ik) - et(n,ik)) < 1e-7) THEN
                    IF (noncolin) THEN
         ...
         do iw=1,n_wannier
            do jw=1, n_wannier
               if( (iw == jw .and. abs(check_mat(iw,jw) - 1) > 1d-3) .or. &
                  ...
                  write(stdout,"(a,i5,a,i5,a)") "compute_mmn_sym: Symmetry operator (", isym, &
                          ") could not transform Wannier function (", iw, ")."
                  write(stdout,"(a,f15.7,a  )") "compute_mmn_sym: The error is ", check_mat(iw,jw) , "."
                  write(stdout,*) "rotmat(:,:,isym)"
                  write(stdout,*) rotmat(:,:,isym)
                  write(stdout,*) "check_mat"
                  write(stdout,*) check_mat
                  call errore("compute_mmn_sym", "Error: missing Wannier functions, see the output.", 1)
               end if
            end do
         end do
      end do
         ...
         do l=1, 3
            do m1 = 1, 2*l+1
               ...
               do m2 = 1, 2*l+1
                  ...
                  mat = sum(dylm1(:)*dylm2(:)*dwgt)*2d0*tpi
   ...
   SUBROUTINE Gk_sym_phase()
      ...
      INTEGER                  :: isym
      REAL(DP)                 :: k(3), sk(3), kdiff(3)
      COMPLEX(DP), ALLOCATABLE :: spsi(:,:), gpsi(:,:), phase(:)
      !
      ALLOCATE (spsi(npwx*npol,nbnd), gpsi(npwx*npol,nbnd))
      ALLOCATE (phase(nbnd))
      !
      DO ik=1, iknum
         ...
         ELSE
            spsi = evc
         END IF
         ! look for first symmetry operation having T in Gk
         DO isym=1, nsym2
            ! check k = s.k + G(kdiff)
            sk = matmul(s2(:,:,isym), k)
            IF (t_rev2(isym) == 0) CYCLE
            sk = -sk
            kdiff = k - sk
            IF (any(abs(kdiff - nint(kdiff)) > 1e-5)) CYCLE
            CALL rotate_evc(isym, ik, ik, kdiff, evc, gpsi)
            ...
            DO m=1, nbnd
               IF (excluded_band(m)) CYCLE
               IF (noncolin) THEN
                  phase(m) = zdotc(npw, spsi(1,m), 1, gpsi(1,m), 1) &
                           + zdotc(npw, spsi(npwx+1,m), 1, gpsi(npwx+1,m), 1)
               ELSE
                  phase(m) = zdotc(npw, spsi(1,m), 1, gpsi(1,m), 1)
               END IF
            END DO
            CALL mp_sum(phase, intra_pool_comm)
            phase = (phase/abs(phase))**0.5
            DO m=1, nbnd
               IF (excluded_band(m)) CYCLE
               evc(:,m) = evc(:,m)*phase(m)
            END DO
   ...
   SUBROUTINE rotate_becp(isym, sgn_sym, xk0, xk, becp1, becp2)
      ...
      DO ia = 1,nat
         ...
         !tau_phase = -tpi*( sgn_sym*SUM((tau(:,ia)-tau(:,ma))*xk0) )

         !tau_phase = -tpi*( sgn_sym*SUM((tau(:,ia)-rau(:,ma))*xk0) )
         !tau_phase = -tpi*( sgn_sym*SUM((tau(:,ia)-rau(:,ia))*xk0) )
         tau_phase = -tpi*( sgn_sym*SUM((tau(:,ma)-rau(:,ia))*xk0) )

         !tau_phase = tpi*( sgn_sym*SUM((tau(:,ia)-rau(:,ma))*xk0) )
         !tau_phase = -tpi*( sgn_sym*SUM(tau(:,ia)*xk0) - SUM(rau(:,ia)*xk0) )
         ...
         DO ih = 1, nh(nt)
            ...
            DO m_o = 1, 2*l_i +1
               ...
               IF (noncolin) THEN
                  IF (sgn_sym > 0) THEN
                     !becp2%nc(ikb, :, :) = becp2%nc(ikb, :, :) &
                     !  + matmul(u_spin, D(l_i)%d(m_o,m_i, isym0) * tau_fact*becp1%nc(okb, :, :))
                     becp2%nc(okb, :, :) = becp2%nc(okb, :, :) &
                       + matmul(u_spin, D(l_i)%d(m_i,m_o, isym0) * tau_fact*becp1%nc(ikb, :, :))
                  ELSE
                     !becp2%nc(ikb, :, :) = becp2%nc(ikb, :, :) &
                     !  + matmul(u_spin, D(l_i)%d(m_o,m_i, isym0) * tau_fact*CONJG(becp1%nc(okb, :, :)))
                     becp2%nc(okb, :, :) = becp2%nc(okb, :, :) &
                       + matmul(u_spin, D(l_i)%d(m_i,m_o, isym0) * tau_fact*CONJG(becp1%nc(ikb, :, :)))
                  ENDIF
               ELSE
                  IF (sgn_sym > 0) THEN
                     !becp2%k(ikb, :) = becp2%k(ikb, :) &
                     !  + D(l_i)%d(m_o,m_i, isym0) * tau_fact*becp1%k(okb, :)
                     becp2%k(okb, :) = becp2%k(okb, :) &
                       + D(l_i)%d(m_i,m_o, isym0) * tau_fact*becp1%k(ikb, :)
                  ELSE
                     !becp2%k(ikb, :) = becp2%k(ikb, :) &
                     !  + D(l_i)%d(m_o,m_i, isym0) * tau_fact*CONJG(becp1%k(okb, :))
                     becp2%k(okb, :) = becp2%k(okb, :) &
                       + D(l_i)%d(m_i,m_o, isym0) * tau_fact*becp1%k(ikb, :)
                  ENDIF
               END IF
   ...
   !
END SUBROUTINE compute_mmn_sym
