         ...
         DO i = 1, nat
            ...
            DO nwfc = 1, atproj_typs(it)%nproj
               ...
               DO m = 1, 2*l+1
                  ...
                  WRITE (stdout, '(" m=", i2, ")")') m
                  ! Set center_w, l_w, mr_w for this projector
                  center_w(1:3, n_proj) = tau(1:3, i)  ! Atom position in crystal coords
                  l_w(n_proj) = l
                  mr_w(n_proj) = m
                  ! Set default axes and rotation angle
                  zaxis(1:3, n_proj) = [0.0_DP, 0.0_DP, 1.0_DP]  ! z-axis
                  xaxis(1:3, n_proj) = [1.0_DP, 0.0_DP, 0.0_DP]  ! x-axis
                  alpha_w(n_proj) = 0.0_DP
                  r_w(n_proj) = 1.0_DP
               ENDDO
            ENDDO
         ENDDO
         WRITE (stdout, *) ''
         ! Convert wannier centers to cartesian coordinates
         !DO i = 1, n_proj
         !   CALL cryst_to_cart(1, center_w(:,i), at, 1)
         !ENDDO
      ELSE
