...
SUBROUTINE read_nnkp
  ...
  CALL mp_bcast(n_wannier,ionode_id, world_comm)
  IF (atom_proj) THEN
     ! auto_projections initially allocates these arrays with n_proj=0.
     ! Resize all projector metadata consistently on every MPI rank.
     DEALLOCATE(center_w, alpha_w, l_w, mr_w, r_w, zaxis, xaxis)
     ALLOCATE(center_w(3,n_wannier), alpha_w(n_wannier), &
              l_w(n_wannier), mr_w(n_wannier), r_w(n_wannier), &
              zaxis(3,n_wannier), xaxis(3,n_wannier), stat=ierr)
     IF (ierr /= 0) CALL errore('read_nnkp', &
          'Error reallocating atom-projection arrays', 1)
     center_w = 0.0_DP
     alpha_w = 0.0_DP
     l_w = 0
     mr_w = 0
     ...
     IF (noncolin) THEN
        ...
        ALLOCATE(spin_eig(n_wannier), spin_qaxis(3,n_wannier), stat=ierr)
        IF (ierr /= 0) CALL errore('read_nnkp', &
             'Error reallocating atom-projection spin arrays', 1)
        spin_eig = 1
        spin_qaxis = 0.0_DP
        spin_qaxis(3,:) = 1.0_DP
     ENDIF
...
SUBROUTINE compute_mmn_ibz
   ...
   SUBROUTINE save_sym_info()
      ...
      USE wannier,         ONLY : seedname, n_wannier, nexband, center_w
      !
      INTEGER                  :: i, m, n, iun_sym, mi, ni, a, ncount
      CHARACTER (len=9)        :: cdate,ctime
      REAL(DP)                 :: srt(3,3)
      COMPLEX(DP)              :: tr, u_spin(2,2)
      REAL(DP), ALLOCATABLE    :: center_frac(:,:)
      ...
      IF (ionode) THEN
         ...
         END DO
         !
         ! save Wannier centers (crystal coordinates)
         !
         ALLOCATE(center_frac(3, n_wannier), stat=ierr)
         IF (ierr /= 0) CALL errore('pw2wannier90', 'Error allocating center_frac', 1)
         center_frac = center_w
         CALL cryst_to_cart(n_wannier, center_frac, bg, -1)

         write(iun_sym, *)
         WRITE(iun_sym,*) 'Wannier centers (crystal)'
         WRITE(iun_sym,*) n_wannier
         DO m=1, n_wannier
            WRITE(iun_sym, '(3f20.12)') center_frac(:, m)
         END DO
         DEALLOCATE(center_frac)
         CLOSE(iun_sym)
      END IF
   END SUBROUTINE
   !
   ...
   SUBROUTINE get_rotation_matrix(rotmat)
      ...
      IF (atom_proj) THEN
         DO iw = 1, n_wannier
            ! Keep atom_proj AMN unchanged and absorb the
...
SUBROUTINE compute_amn_with_atomproj
   ...
   USE mp_world, ONLY: world_comm
   USE io_global, ONLY: stdout, ionode, ionode_id
   USE ions_base, ONLY: nat, ityp, atm, nsp, tau
   USE basis, ONLY: natomwfc, swfcatom
   ...
   REAL(DP), ALLOCATABLE :: center_w_ext(:,:)
   INTEGER, ALLOCATABLE :: l_w_ext(:), mr_w_ext(:), spin_eig_ext(:)
   ...
   IF (ionode) THEN
      IF (atom_proj_ext) THEN
         ...
         ! Count all external projectors before applying atom_proj_exclude.
         natproj = 0
         DO i = 1, nat
            it = ityp(i)
            DO nwfc = 1, atproj_typs(it)%nproj
               IF (lspinorb) THEN
                  natproj = natproj + atproj_typs(it)%twoj(nwfc) + 1
               ELSE
                  natproj = natproj + 2*atproj_typs(it)%l(nwfc) + 1
               ENDIF
            ENDDO
         ENDDO
         IF (.NOT. lspinorb) THEN
            ALLOCATE(center_w_ext(3,natproj), l_w_ext(natproj), &
                     mr_w_ext(natproj), stat=ierr)
            IF (ierr /= 0) CALL errore('compute_amn_with_atomproj', &
                 'Error allocating external-projector metadata', 1)
            IF (noncolin) THEN
               ALLOCATE(spin_eig_ext(natproj), stat=ierr)
               IF (ierr /= 0) CALL errore('compute_amn_with_atomproj', &
                    'Error allocating external-projector spin metadata', 1)
            ENDIF
         ENDIF
         !
         n_proj = 0
            ...
            ELSE IF (noncolin) THEN
               DO nwfc = 1, atproj_typs(it)%nproj
                  ...
                  DO m = 1, 2*l + 1
                     ...
                     center_w_ext(:,n_proj) = tau(:,i)
                     l_w_ext(n_proj) = l
                     mr_w_ext(n_proj) = m
                     spin_eig_ext(n_proj) = atproj_typs(it)%twosz(nwfc)
                  ENDDO
               ENDDO
            ELSE
               DO nwfc = 1, atproj_typs(it)%nproj
...
1000  FORMAT(5X, "state #", i4, ": atom ", i3, " (", a3, "), wfc ", i2, &
      ...
      ENDIF
      IF (n_proj /= n_wannier) THEN
         WRITE(stdout, '(5x,a,i0,a,i0,a,i0)') 'total projectors = ', natproj, &
              ', excluded = ', nexatproj, ', n_wannier = ', n_wannier
         CALL errore('compute_amn_with_atomproj', &
              'number of retained atomic projectors does not match n_wannier', 1)
      ENDIF
      !
      ...
      IF (atom_proj_ext) THEN
         IF (lspinorb) THEN
            IF (irr_bz) CALL errore('compute_amn_with_atomproj', &
                 'atom_proj_ext with SOC is not supported for irr_bz', 1)
         ELSE
            ! Compact metadata from the full external-projector list.
            DO i = 1, n_proj
               nwfc = proj_excl_map(i)
               center_w(:,i) = center_w_ext(:,nwfc)
               l_w(i) = l_w_ext(nwfc)
               mr_w(i) = mr_w_ext(nwfc)
               zaxis(:,i) = [0.0_DP, 0.0_DP, 1.0_DP]
               xaxis(:,i) = [1.0_DP, 0.0_DP, 0.0_DP]
               alpha_w(i) = 0.0_DP
               r_w(i) = 1.0_DP
               IF (noncolin) THEN
                  spin_eig(i) = spin_eig_ext(nwfc)
                  spin_qaxis(:,i) = [0.0_DP, 0.0_DP, 1.0_DP]
               ENDIF
            ENDDO
            DEALLOCATE(center_w_ext, l_w_ext, mr_w_ext)
            IF (ALLOCATED(spin_eig_ext)) DEALLOCATE(spin_eig_ext)
         ENDIF
      ELSE
         ! Store UPF metadata in the same retained-projector order as wfcatom.
         DO i = 1, n_proj
            nwfc = proj_excl_map(i)
            center_w(1:3, i) = tau(1:3, nlmchi(nwfc)%na)
            l_w(i) = nlmchi(nwfc)%l
            mr_w(i) = nlmchi(nwfc)%m
            zaxis(1:3, i) = [0.0_DP, 0.0_DP, 1.0_DP]
            xaxis(1:3, i) = [1.0_DP, 0.0_DP, 0.0_DP]
            alpha_w(i) = 0.0_DP
            r_w(i) = 1.0_DP
            IF (noncolin .AND. .NOT. lspinorb) THEN
               spin_eig(i) = 1 - 2*INT(nlmchi(nwfc)%ind/(2*nlmchi(nwfc)%l + 2))
               spin_qaxis(:,i) = [0.0_DP, 0.0_DP, 1.0_DP]
            ENDIF
         ENDDO
      ENDIF
   ...
   CALL mp_bcast(n_proj, ionode_id, world_comm)
   ! The root rank populated these arrays from the atomic projectors above.
   CALL mp_bcast(center_w, ionode_id, world_comm)
   CALL mp_bcast(l_w, ionode_id, world_comm)
   CALL mp_bcast(mr_w, ionode_id, world_comm)
   CALL mp_bcast(r_w, ionode_id, world_comm)
